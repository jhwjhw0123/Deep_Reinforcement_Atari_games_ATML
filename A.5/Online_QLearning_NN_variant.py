...
Load_path = '../../Data/A.4/'
Model_path = '../../Model/A.5/Model/'
Save_path = '../../result/A.5/'
...
def neural_net_work(s_current,s_next,r):
    tf.set_random_seed(16674)
    Weights = {'hidden':tf.Variable(tf.random_normal([4,30])),\
               'output':tf.Variable(tf.random_normal([30,2]))}
    Bias = {'hidden':tf.Variable(tf.random_normal([30])),\
...
def online_train_neural_network(s_current,r):
    ...
    # optimiser = tf.train.RMSPropOptimizer(learning_rate=LR,decay=0.9,momentum=0.1,centered=True).minimize(loss)
        ...
        for c_episode in range(n_episode):
            ...
            for t in range(episode_len):
                ...
                if done == True:
                    ...
                    this_discounted_reward = math.pow(Gamma,run_eps_len-1)*(-1)
                    reward_dis_list.append(this_discounted_reward)
                    if c_episode % 20 == 0 and c_episode != 0:
                        ...
                        discounted_rewards.append(np.mean(reward_dis_list))
                        loss_list = []
                        reward_dis_list = []
                        episode_list = []
...
# np.save(Save_path+'episode_performance_30',episode_plot)
# np.save(Save_path+'loss_30',loss_plot)
# np.save(Save_path+'reawrd_30',discounted_rewards)
episode_plot = np.load(Save_path+'episode_performance_1000'+'.npy')
loss_plot = np.load(Save_path+'loss_1000'+'.npy')
discounted_rewards = np.load(Save_path+'reawrd_1000'+'.npy')
# plt.subplot(1,3,1)
