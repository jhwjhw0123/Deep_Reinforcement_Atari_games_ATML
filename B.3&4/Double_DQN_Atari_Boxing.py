...
def convolutional_neural_network(s_current,s_next,r):
    ...
    #Primary network for next state
    # Process current state
    layer1_convolved_next_primary = convolution2D(s_next, Weights['conv_layer1'])
    layer1_convolved_next_primary = tf.nn.relu(layer1_convolved_next_primary)

    layer2_convolved_next_primary = convolution2D(layer1_convolved_next_primary, Weights['conv_layer2'])
    layer2_convolved_next_primary = tf.nn.relu(layer2_convolved_next_primary)

    flatten_next_primary = tf.reshape(layer2_convolved_next_primary, [-1, (img_size // 4) * (img_size // 4) * n_channel_layer2])

    full_connect_next_primary = tf.add(tf.matmul(flatten_next_primary, Weights['fully_connect_layer']),
                                  Bias['fully_connect_layer'])
    full_connect_next_primary = tf.nn.relu(full_connect_next_primary)

    output_next_primary = tf.add(tf.matmul(full_connect_next_primary, Weights['output']), Bias['output'])
...
def online_train_neural_network(s_current,r):
    prediction_current,prediction_next,prediction_next_primary,reward_tensor = convolutional_neural_network(s_current,s_next,r)
    ...
    with tf.Session() as sess:
        ...
        for c_episode in range(n_episode):
            ...
            for t in range(episode_len):
                ...
                if total_step<buffer_len or t<4:
            ...
            if total_step > 2000000:
                # save_model(sess)
                break
        if total_score_collection[-1]>=20:
            save_model(sess)
            np.save(Save_path+'Bellman_loss',np.asarray(Bellman_loss_collection))
            np.save(Save_path+'raw_score',np.asarray(total_score_collection))
            np.save(Save_path + 'discounted_reward',np.asarray(discounted_reward_collection))
